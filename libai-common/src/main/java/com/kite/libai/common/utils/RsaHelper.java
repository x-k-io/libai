...
public class RsaHelper {
    ...
    private static final byte[] SEQ_OID = new byte[]{0x30, 0x0D, 0x06, 0x09, 0x2A, (byte) 0x86, 0x48, (byte) 0x86,
    ...
    // 以下参数只有私钥才有
    // https://docs.microsoft.com/zh-cn/dotnet/api/system.security.cryptography.rsaparameters?redirectedfrom=MSDN&view=netframework-4.8
    /**
     * prime1
     **/
    private byte[] valP;
    ...
    public RsaHelper(RSAPublicKey publicKey, RSAPrivateKey privateKeyOrNull) {
        this(
                bigB(publicKey.getModulus()), bigB(publicKey.getPublicExponent()),
     ...
     */
    public RsaHelper(byte[] modulus, byte[] exponent, byte[] d, byte[] p, byte[] q, byte[] dp, byte[] dq,
                     byte[] inverseQ) {
        keyModulus = modulus;
     ...
     * 将RSA中的密钥对转换成PEM格式
     * 。convertToPublic：等于true时含私钥的RSA将只返回公钥，仅含公钥的RSA不受影响
     * 。privateUsePKCS8：私钥的返回格式，等于true时返回PKCS#8格式（-----BEGIN PRIVATE KEY-----），否则返回PKCS#1格式（-----BEGIN
     * RSA PRIVATE KEY-----），返回公钥时此参数无效；两种格式使用都比较常见
     * 。publicUsePKCS8：公钥的返回格式，等于true时返回PKCS#8格式（-----BEGIN PUBLIC KEY-----），否则返回PKCS#1格式（-----BEGIN RSA
     * PUBLIC KEY-----），返回私钥时此参数无效；一般用的多的是true PKCS#8格式公钥，PKCS#1格式公钥似乎比较少见
     */
    ...
    public static RsaHelper fromXml(String xml) {
        ...
        if (rtv.keyD != null) {
            if (rtv.valP == null || rtv.valQ == null || rtv.valDP == null || rtv.valDQ == null
                    || rtv.valInverseQ == null) {
